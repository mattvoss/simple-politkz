          ...
          var question = function(question, cb2) {
            if (question.topic === race.topic && question.chart === race.chart) {
              var data  = {
                    ...
                    updated: moment.utc(poll.last_updated).format("YYYY-MM-DD HH:mm:ss"),
              ...
              async.each(question.subpopulations[0].responses, sub, function(err) {
                ...
                console.log("Chart: ", question.chart);
                console.log("Pollster: ", poll.pollster);
                console.log("Start Date: ", poll.start_date);
                console.log("End Date: ", poll.end_date);
                console.log("Updated: ", poll.last_updated);
                ...
                connection.query('SELECT id FROM polls WHERE pollsterId = ?', [data.pollsterId], function(err, rows) {
                  if (rows.length === 0) {
                    ...
                    data.id = rows[0].id;
                    connection.query('UPDATE polls SET ? WHERE pollsterId = ?', [data, data.pollsterId], function(err, result) {
                      console.log(err);
                      console.log(result);
                      cb2();
                    });
